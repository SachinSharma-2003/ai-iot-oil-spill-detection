...
# st.title("🛢️ AI-Driven Oil Spill Detection System")
# st.write(
#     "Upload a SAR image to detect oil spill regions using a U-Net segmentation model."
# )
...
#     model.load_weights(
#         r"C:\ml-models\unet_oil_spill.weights.h5",
#         by_name=True,
#         skip_mismatch=True
#     )
#     return model
...
# model = st.session_state.model  # ALWAYS defined
...
#         step=0.05,
#         help=(
#             "Lower values detect smaller or weaker oil spills but may include noise. "
#             "Higher values detect only strong, confident oil spill regions."
#         )
#     )
...
#     # -------- WARN USER IF TOO SENSITIVE --------
#     if np.mean(pred_mask) > 0.9:
#         st.warning(
#             "⚠️ High sensitivity detected. Try increasing the slider for clearer spill regions."
#         )
...
#     # -------- COLORED MASK --------
#     st.subheader("Predicted Oil Spill Mask (Cleaned)")

#     colored_mask = np.zeros((256, 256, 3), dtype=np.uint8)
#     colored_mask[pred_mask.squeeze() == 1] = [255, 0, 0]  # RED spill

#     st.image(colored_mask, caption="Detected Oil Spill Regions (Red)")

#     # -------- CONFIDENCE --------
#     confidence = float(np.mean(pred))
#     st.write(f"**Average Oil Spill Probability:** {confidence:.3f}")
